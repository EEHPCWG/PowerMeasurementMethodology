...
# rpeak per node: (128 cores per SM) * (132 SM per GPU) * (4 GPUs per node) * (1.980 GHz FP64 tensor core boost frequency) * (1 multiply + 1 add)
rpeak_gflops_per_node = 128 * 132 * 4 * 1.980 * 2
...
power_kilowatt_limit = 300

# time resolution needed for detecting missing values
pdus_resolution = "5s"
# configure idle phase reference time frame
idle_start = int(datetime(2024, 9, 27, 8, 15, 0, tzinfo=ZoneInfo("Europe/Berlin")).timestamp())
idle_end = int(idle_start + timedelta(minutes=15).total_seconds())
# control input / output path
...
jobid = 49525701
job_start, job_end = 1727428575, 1727428949
num_nodes = 36
racks = {
    # compute racks
    245,
...
##################################################
# double reported energy consumption of 2 Isilon PDUs as the respective other PDUs are broken which consumed around the same energy
pdus = pdus.with_columns(pl.col("^443#2|444#1$").diff().mul(2).cum_sum().fill_null(0))
...
power_watt_job = power_watt_job_pdus
...
power_watt_core = power_watt_core_pdus
...
power_watt_idle = power_watt_idle_pdus
...
##################################################
# no alignment necessary as all PDUs collect in a synchronized manner
all_aligned = pdus_sum_job
