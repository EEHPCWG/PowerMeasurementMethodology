...
Average power values should be reported in kilowatts (kW).

\subsection{Accuracy Requirements}
\label{sec:accuracy}

For Level~3, it is required to use any revenue grade meter, any meter accepted by SPECpower, or any meter documented to have an accuracy of \SpecAccuracyLThree{} or better.
...
To ensure the above accuracy requirements are met for Level~3, the \ac{EE HPC WG} recommends, but does not require, revenue grade meters certified for ANSI C12.20, IEC 62053-21 (classes~0,5, and~1), or IEC 62053-22 (classes~0,1S, 0,2S, and~0,5S).
Additionally, any meter accepted by SPECpower is also permitted for Level~3.
For further information, refer to the SPECpower \textit{Power and Temperature Measurement Setup Guide}\footnote{\url{https://www.spec.org/power/docs/SPEC-Power_Measurement_Setup_Guide.pdf}} and the associated list of accepted power measurement devices\footnote{\url{https://www.spec.org/power/docs/SPECpower-Device_List.html}}.
...
The readout interval should be approximately constant for the duration of the observation, see Section~\ref{sec:accuracy}.
...
\subsubsection{Deriving Average Power Using Instantaneous Power Samples}

The primary reported power values used for the efficiency calculation are aggregate average power values across all participating subsystems, see Section~\ref{sec:A3SIiIP}. over the core phase or the full run (\emph{observation interval}).
...
While the main ranking is computed from the overall average power value, submitters are highly recommended to include the series of individual power values and their timestamps in the submission, e.g., as a csv file.

\subsubsection{Deriving Average Power Using Continuous Integrated Energy}
...
Using integrated energy values is highly recommended, when available.
The reported value is still the aggregate average power over all participating subsystems, which is computed by dividing the difference in energy measurements $E_\text{end}$ and $E_\text{begin}$ by the difference in timestamps of the last reading $t_\text{end}$ and the first reading $t_\text{begin}$ within the observation interval.
...
% \textbf{2: Machine \newline fraction}  &
% The entire system; at least \SpecPowerMaxLOne{}; or the largest of: \SpecFracMinLOne{} of the system or at least one rack of the system &
% The greater of \SpecFracMinLTwo{} of the compute-node subsystem or at least one rack of the system &
...
Since \acs{HPL} only reports the time spent executing the core phase of the workload, the time for the total run must be measured and reported separately by the submitter, for example by prepending the UNIX time command to the job invocation or the parallel application launch, whichever is earlier.
...
Submitters are encouraged to supply a detailed set of measurements that were are used to calculate the average power values and that showcase the dynamic power consumption over the course of the benchmark.
Such time series of power or energy values can be attached to the submission, for example as CSV file.

The submitter may find it useful to create a graph showing the power and energy during the workload as shown in Figure~\ref{fig:example-rwth-claix-2023-summary}.
...
Across all levels, all compute nodes and subsystems participating in the workload must be included when reporting power.
Full system measurements better reflect how machines actually operate and utilize existing continuous monitoring investments.
...
% \item Whichever is largest of: \SpecFracMinLOne{} of the system or at least one rack of the system
...
% \item
% Level~2: The largest of \SpecFracMinLTwo{} of the compute-node subsystem or at least one rack of the system.
...
Subsystems include computational nodes, any interconnect network the application uses, any head or control nodes, and any internal cooling devices (self-contained liquid cooling systems and fans).
...
For Level~1 and Level~2, the compute-node subsystem power must be measured.
All other subsystems participating in the workload must also be measured or, if not measured, the contribution must be estimated.
...
The reported power measurement must include all computational nodes, any interconnect network the application uses, any head or control nodes, all power conversion losses inside the computer, and any internal cooling devices (self-contained liquid cooling systems and fans).
