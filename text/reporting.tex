...
Across all levels, all participating subsystems in the workload must be included when reporting power.
Using a subset of nodes results in overall better performance or energy efficiency, for example to improve HPL decomposition, is acceptable.
The submission of power-optimized runs that are separate from runs submitted to the Top500 are allowed.

Full system measurements better reflect how machines actually operate and utilize existing continuous monitoring investments rather than relying on one-off instrumentations.
