...
This reported power value may consist of several power measurements taken at a constant rate of at least once per second and averaged over an interval. 
...
\noindent
Level~2 also requires that power measurements be taken at a constant rate of at least once per second.
...
Each of the measurements required for L2 must power-average over the entire interval between two reported measurements.
\wl
...
\noindent
For example, the meter may sample at one-second intervals and report a value every minute. Assume that the core phase is 600 minutes long. Assume further that the requirement for 10  measurements can be satisfied with 10 measurements at an interval of 50 minutes. Using just those 10 measurements does not conform to this specification because not all the values reported by the meter during the core phase are used.
\wl

\noindent
Although those measurements were reported at an interval of 50 minutes, each averaged only over a minute. So a portion of the time interval between the measurements was not included in the average. 
\wl
...
\noindent
Level~3 reports an average power value for the core phase, an average power value for the whole run, at least 10 energy values at regular intervals within the core phase, and the elapsed time between the initial and final energy readings in the core phase. The average power value for the core phase is the difference between the initial and final energy readings divided by the elapsed time.

\section{Aspect and Quality Levels}
\label{sec:AQLevels}
\noindent
Table~\ref{tab:levels} summarizes the aspect and quality levels introduced in Section~\numnameref{sec:intro}
...
The largest of 1/8 of the compute-node subsystem, 10~kW, or 15~compute nodes  &
...
\noindent
When calculating the average power of the full machine for Levels 1 and 2, the measured power must be multiplied by this fraction to 
...
\item
L2: the largest of $ \frac{1}{8} $ of the compute-node subsystem, 10 kW, or 15~compute nodes
\item
