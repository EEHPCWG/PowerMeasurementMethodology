...
Across all levels, all participating compute nodes and all participating subsystems must be included when reporting power.
Full system measurements better reflect how machines actually operate and utilize existing continuous monitoring investments rather than relying on one-off measurement instrumentation.
Note that submissions are allowed use a subset of nodes to improve performance or energy efficiency, for example due to an improved HPL decomposition.
