...
It is allowed that the measurement frequency is not strictly constant, but it can be adjusted such that each measurement interval covers a certain number of full sinus waves.
In that case, the intervals must not exceed one second by more than one sinus wave and when building the final power average the power readings must be weighted by the duration of the measurement interval.
\wl
