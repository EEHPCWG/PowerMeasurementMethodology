...
For Level 1, the only subsystem the only subsystem that requires an actual power measurement is the compute-node subsystem is the compute-node subsystem. 
...
+must be estimated. 
Include everything that you need to operate the network that is not part of the compute subsystem. 
